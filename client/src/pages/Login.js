...
export default function SignIn()
{
    ...
    if (isPasswordError)
    {
        passwordFieldProps = {
            error: true,
            helperText: "Password must be at least 5 characters long before submission."
        }
    }
        ...
        <Container component="main" maxWidth="xs">
            ...
            <div className={classes.paper}>
                ...
                <form className={classes.form} onSubmit={(e) =>
                {
                    ...
                    {!isPasswordForgotten && <Fragment>
                        <Grid container spacing={3}>
                            {providers.map(provider => <Grid key={provider} item xs={12} sm={6} md={6} lg={6}>
                                <OAuth
                                    provider={provider}
                                    socket={socket}
                                />
                    ...
                    <Grid container className={classes.submit}>
                        <Grid item xs>
                            {isPasswordForgotten ?
                                <span style={{ textDecoration: "underline", cursor: "pointer" }} onClick={() => swapLoginForgotPassword()}>
                                    Back to login
                            </span> :
                                <span style={{ textDecoration: "underline", cursor: "pointer" }} onClick={() => swapLoginForgotPassword()}>
                                    Forgot password?
