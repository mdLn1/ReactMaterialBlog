...
export default function Register()
{
    ...
    {
        toggleLoading(true)
        if (isEmailAddressValid(emailAddress) && isPasswordValid(password) && isUsernameValid(username))
        {
            const data = {
                email: emailAddress,
                password,
    ...
    if (isPasswordError)
    {
        ...
        passwordFieldProps = {
            error: true,
            helperText: "Password must be at least 8 characters long and contain minimum 1 uppercase letter, 2 lowercase letters, 2 digits and 1 symbol."
        }
    }
    if (isUsernameError)
    {
        ...
        usernameFieldProps = {
            error: true,
            helperText: "Username must contain at least 2 letters and be minimum 5 characters long."
        }
    }
                ...
                <form className={classes.form} onSubmit={(e) =>
                {
                    ...
                    {!isEmailAddressError &&
                        <FormControl component="fieldset">
                            <FormLabel component="legend">Your Email address visibility</FormLabel>
                            <RadioGroup row aria-label="position" name="position" defaultValue="public">
                                <FormControlLabel
                                    value="public"
                                    control={<Radio color="primary" onClick={() => toggleDisplayEmail(true)} />}
                                    label="Public"
                                    labelPlacement="start"
                                />
                                <FormControlLabel
                                    value="private"
                                    control={<Radio color="primary" onClick={() => toggleDisplayEmail(false)} />}
                                    label="Private"
                                    labelPlacement="start"
                                />
                            </RadioGroup>
                    ...
                    />
                    <FormControlLabel
                        control={<Checkbox value="allowExtraEmails" color="primary" onChange={() => toggleReceiveUpdates(val => !val)} />}
